...
/**
 * Pure JavaScript SHA-256 (FIPS 180-4) for runtimes without a crypto module.
 * The plugin target is IINA's JavaScriptCore, so this file must stay free of
 * Node and Web platform APIs, including TextEncoder.
 */
...
function hashBytes(bytes) {
  ...
  for (let offset = 0; offset < paddedLength; offset += 64) {
    ...
    for (let t = 0; t < 64; t += 1) {
      ...
      const temp1 = (h + bigS1 + choose + K[t] + w[t]) >>> 0;
...
function sha256Hex(text) {
  return hashBytes(utf8Bytes(text));
}
